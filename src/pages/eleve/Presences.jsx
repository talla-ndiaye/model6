  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen py-8">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <h2 className="text-xl font-bold text-gray-800 pt-4 pb-3">Vue d'overview de mon assiduité</h2>
        <div className="relative -mx-2">
          <Slider {...sliderSettings}>
            {statItems.map((item, idx) => (
              <div key={idx} className="px-2">
                <Card className={`p-5 text-center shadow-lg rounded-xl border-b-4 ${item.borderColor} bg-white h-full flex flex-col justify-center`}>
        ...
        <h2 className="text-xl font-bold text-gray-800 pt-4 pb-3">Mes Absences-Reatrds-Renvois</h2>
        ...
        <Modal isOpen={isModalOpen} onClose={closeDetailsModal} title={`Détails du ${new Date(selectedPresenceDetails?.date || '').toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}`} size="md">
          {selectedPresenceDetails && (
            <div className="space-y-4 text-gray-700 p-3">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-blue-50 p-4 rounded-lg border border-blue-200">
                <div>
