import { AlertCircle, Calendar, CheckCircle, Clock, User, UserX, XCircle } from 'lucide-react';
...
const PresencesEnfants = () => {
  ...
  const presencesEnfant = selectedChild 
    ? presences.filter(presence => presence.eleveId === parseInt(selectedChild))
    : [];

  // Filtrer par mois sélectionné
  const presencesDuMois = presencesEnfant.filter(presence => 
  ...
  const getStats = () => {
    ...
    const justifies = presencesEnfant.filter(p => p.justifie && p.statut !== 'present').length;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Présences de mes enfants</h1>
        <p className="text-gray-600">Suivez l'assiduité de vos enfants</p>
      </div>
            ...
            <>
              ...
              <Card className="p-6">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">
                  Informations - {getChildName(selectedChild)}
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    <p className="text-sm text-gray-600">Taux d'assiduité</p>
                    <p className={`font-medium ${
                      parseFloat(stats.tauxPresence) >= 95 ? 'text-green-600' :
                      parseFloat(stats.tauxPresence) >= 85 ? 'text-orange-600' : 'text-red-600'
                    }`}>
                      {stats.tauxPresence}%
                    </p>
                  </div>
